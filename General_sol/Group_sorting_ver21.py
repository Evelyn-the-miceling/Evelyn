...
def Groups_of_5(student_list, n):
    ...
    for i in range(Leftovers):
        Leftover_student_list.append(student_list.pop()) 
        #Pop the final few students to ensure that our groups will have the same number of students
    ...
    if n % 2 == 1: #Odđ membered group size
        ...
        num_of_n_minus_1_people_groups = len(all_subgroups_sorted)
        num_of_remaining_students = len(Unsorted_Students)

        for i in range(min(num_of_n_minus_1_people_groups, num_of_remaining_students)): 
            if len(all_subgroups_sorted[i]) == n - 1: #To ensure that subgroups are addable
...
def WriteOutput(Output_groups, filename = "OutputFile.csv"): #Change the name into FDAE_Group4_<TeamLeaderName> when we are done coding!
    try:
        with open(filename, mode='w') as file:
            header_row = ",".join(["Tutorial Group", "Student ID", "School", "Name", "Gender", "CGPA", "Team Assigned"]) + "\n"
            file.write(header_row)
            for i in range(120): #Take out each tutorial group in output file in ascending order
                for allocated_groups in Output_groups[f"G-{i + 1}"]: #Take out each allocated group in the tutorial group
